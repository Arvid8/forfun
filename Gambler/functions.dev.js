...
module.exports = function (con, client) {
  var ranks = require("./Games/Manager/ranks"); // ------------------------- SQL Management -------------------------
  ...
  this.checkBet = function (msg, userID, bet, isWager, addToTotal, callback) {
    checkBalance(userID, function (result) {
      if (result >= bet && bet >= 10) {
        checkRankUpgrade(userID, bet, addToTotal, function () {
          // Rank upgrade
          loss(userID, bet, isWager, function (newbal) {
            return callback({
              canPlay: true,
              bal: newbal
            });
  ...
  this.tryForJackpot = function (player, bet) {
    getGamblingRole(player, function (topRole) {
      var chanceToWin = topRole.jackpot / 100; //In percent
      //const chanceToWin = 50 / 100; //In percent

      var maxRolls = 1 / chanceToWin;
      var winner = getRandomInt(maxRolls);

      if (winner === 1) {
        console.log('MaxRolls: ' + maxRolls + ', chanceToWin: ' + chanceToWin + ' & winner: ' + winner);
        console.log(winner);
        var channel = client.channels.cache.get('734004487382892554');
  ...
  this.checkRankUpgrade = function (userID, bet, addToTotal, callback) {
    if (addToTotal) {
      addTotBet(userID, bet, function (totBet) {
        var n = 1;
        totBet /= 1000;
        ...
        getGamblingRole(userID, function (topRole) {
  ...
  this.upradeRank = function (userID, n) {
    ...
    var guild = client.guilds.cache.get('732675304228651109');
    var member = guild.member(user);
    member.roles.cache.forEach(function (e, i) {
      ranks.forEach(function (e2, i2) {
        if (e2.rank > 1 && e.id == e2.id) {
          member.roles.remove(e);
        }
      });
    });
    var role = ranks.find(function (item) {
      return item.rank == n;
    });
    var roleToAdd = guild.roles.cache.find(function (item) {
      return item.id == role.id;
    });
    member.roles.add(roleToAdd);
    var channel = client.channels.cache.get('734842887786987582');
    channel.send(userID + ' ' + role.id); // Send information to manager to post
  };

  this.getGamblingRole = function (playerID, callback) {
    var user = client.users.cache.get(playerID),
  ...
  this.getUserFromMention = function (mention) {
    ...
    var matches = mention.match(/^<@!?(\d+)>$/); // If supplied variable was not a mention, matches will be null instead of an array.

    if (!matches) return false; // However the first element in the matches array will be the entire mention, not just the ID,
    // so use index 1.
  ...
  this.sendMessage = function (msg, text) {
    msg.channel.send(text)["catch"](function (e) {
      console.log(e);
    });
  };
